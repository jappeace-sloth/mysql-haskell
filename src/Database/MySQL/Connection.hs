...
-- MySQL 8.0 does not accept cleartext passwords over unencrypted TCP.
-- Use TLS connections for full authentication, or ensure the password
-- verifier is cached (fast auth path).
plainFullAuth :: Word8 -> ByteString -> (Packet -> IO ()) -> InputStream Packet -> IO ()
plainFullAuth _ _ _ _ =
    throwIO $ AuthException "caching_sha2_password full authentication requires a TLS connection. Use Database.MySQL.TLS to connect, or configure the MySQL user with mysql_native_password."
